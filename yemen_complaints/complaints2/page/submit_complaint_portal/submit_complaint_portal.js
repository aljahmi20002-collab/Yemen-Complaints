frappe.pages['submit_complaint_portal'].on_page_load = function(wrapper) {
  ...
  wrapper.innerHTML = `
    <div style="padding:2rem;direction:rtl;font-family:DroidArabicKufi,Tahoma,Arial,sans-serif;">
      <div style="max-width:760px;margin:0 auto;background:#fff;border:1px solid #e5e7eb;border-radius:20px;padding:1.5rem;box-shadow:0 12px 30px rgba(15,23,42,.05);">
        ...
        <p><a href="/submit-complaint" style="display:inline-block;padding:.65rem 1rem;border-radius:999px;background:#9b1c1c;color:#fff;text-decoration:none;">افتح الصفحة الآن</a></p>
      </div>
    </div>`;
  setTimeout(() => { window.location.href = '/submit-complaint'; }, 50);
};
